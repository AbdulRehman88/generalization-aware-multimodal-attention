...
JSON_PATH = Path(
    "_research_audit/"
    "bbbd_temporal_matched_24s_metrics_v1.json"
)

CSV_PATH = Path(
    "_research_audit/"
    "bbbd_temporal_matched_24s_metrics_v1.csv"
)


class BBBDTemporalMatchedTwentyFourSecondResultsAuditTests(
    unittest.TestCase
):
    ...
    def test_twenty_four_second_audit_passed(self) -> None:
        self.assertEqual(
            self.audit["status"],
            "passed",
        )
    ...
    def test_metrics_and_intervals_are_bounded(self) -> None:
        metric_columns = [
            ...
            and not column.endswith("_passed")
        ]
    ...
    def test_complete_integrity_coverage(self) -> None:
        self.assertEqual(
            self.audit["report_groups_checked"],
            28,
        )

        self.assertEqual(
            self.audit["bootstrap_rows_checked"],
            140000,
        )

        self.assertEqual(
            self.audit["report_files_checked"],
            142,
        )
