"""Independent numerical audit of matched BBBD sixteen-second reports."""
...
from src.evaluation.bbbd_temporal_matched_results_audit import (
    audit_reports,
)


EXPECTED_MANIFEST_IDENTITY = (
    "bbbd_temporal_matched_16s_reports_v1"
)
...
def audit_sixteen_second_reports(
    ...
    if set(
        ...
        raise RuntimeError(
            "Metric evidence contains an incorrect scientific role."
        )
    ...
    payload.update(
        {
            "audit_identity":
                "bbbd_temporal_matched_16s_metrics_v1",
            "source_commit":
                "4239ac8",
            "status":
                "passed",
            ...
            "temporal_interpretation_boundary":
                (
                    "Sixteen seconds is a prespecified post-primary "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Independently audit matched BBBD sixteen-second reports."
        )
    )
    ...
    payload, metrics = audit_sixteen_second_reports(
    ...
    print(
        "\n===== SIXTEEN-SECOND NUMERICAL AUDIT SUMMARY ====="
    )
    ...
    print(
        "\nINDEPENDENT 16-SECOND NUMERICAL AUDIT PASSED"
    )
