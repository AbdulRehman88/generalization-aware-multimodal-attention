"""Independent numerical audit of matched BBBD thirty-two-second reports."""
...
EXPECTED_MANIFEST_IDENTITY = "bbbd_temporal_matched_32s_reports_v1"
...
def audit_thirty_two_second_reports(
    ...
    expected_manifest_values = {
        ...
        "report_count":
            28,
        "within_experiment_reports":
            14,
        "cross_experiment_reports":
            14,
        "bootstrap_unit":
            "participant",
        "bootstrap_repetitions_per_report":
            5000,
        "bootstrap_seed":
            3407,
        "confidence_level":
    ...
    if len(csv_files) != 113:
        raise RuntimeError(
            f"Expected 113 CSV files, observed {len(csv_files)}."
        )

    if len(json_files) != 29:
        raise RuntimeError(
            f"Expected 29 JSON files, observed {len(json_files)}."
        )
    ...
    if payload.get("status") != "passed":
        raise RuntimeError(
            "Underlying numerical audit did not pass."
        )
    ...
    if set(
        metrics["duration_seconds"].astype(int)
    ) != {32}:
        raise RuntimeError(
            "Metric evidence contains an incorrect duration."
        )
    ...
    payload.update(
        {
            "audit_identity":
                "bbbd_temporal_matched_32s_metrics_v1",
            "source_commit":
                "6ed4745",
            "status":
                "passed",
            ...
            "temporal_interpretation_boundary": (
                "Thirty-two seconds is a prespecified post-primary "
                "temporal-sensitivity duration. It does not replace "
                "the primary four-second analysis and was not selected "
                "post hoc as a preferred operating point."
            ),
...
def main() -> None:
    ...
    payload, metrics = audit_thirty_two_second_reports(
    ...
    print(
        "\n===== THIRTY-TWO-SECOND NUMERICAL AUDIT SUMMARY ====="
    )
    ...
    print(
        "\nINDEPENDENT 32-SECOND NUMERICAL AUDIT PASSED"
    )
